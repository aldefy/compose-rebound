# Not all recompositions are equal
...
<!-- GIF: 15-20s walkthrough — idle app (green) → scroll list (rates spike) → violation fires → Hot Spots tab → Timeline heatmap → editor gutter icon turns red -->
![Rebound IDE plugin in action — live monitoring across all tabs](assets/rebound-ide-walkthrough.gif)
...
<!-- Screenshot: Monitor tab with tree expanded, violation highlighted, event log scrolling -->
![Monitor tab — live composable tree with sparklines and event log](assets/tab-monitor.png)
...
<!-- Screenshot: Hot Spots tab sorted by budget ratio, violations at top -->
![Hot Spots tab — sortable table with summary card](assets/tab-hotspots.png)
...
<!-- Screenshot: Timeline heatmap showing a scroll burst pattern -->
![Timeline tab — composable × time heatmap](assets/tab-timeline.png)
...
<!-- Screenshot: Editor with green/red gutter dots + click popup showing rate/budget/skip -->
![Gutter icons — inline performance indicators in the editor](assets/gutter-icons.png)
...
The repo is live: **[link]**. Add the Gradle plugin, build in debug, and see which of your composables are over budget. The budget numbers come from testing across several Compose apps — if your app has different composition patterns and the defaults don't fit, open an issue. That's how the numbers get better.
